...
void EventAction::BeginOfEventAction(const G4Event *anEvent)
{
	...
	SetReactionZPoint(); //set a new reaction point
...
void EventAction::DecayLab4Vectors(const G4LorentzVector &ParentLV, G4LorentzVector &DecayOut1, G4LorentzVector &DecayOut2)
{
	...
	G4double Pcm = 0.5 * sqrt(fabs(pow(S, 2.0) + pow(mass1, 4.0) + pow(mass2, 4.0) - 2 * S * pow(mass1, 2.0) - 2 * pow(mass1, 2.0) * pow(mass2, 2.0) - 2 * S * pow(mass2, 2.0))) / sqrt(S);
	//fabs() also included here
	//G4cout<<S<<"  "<<Pcm<<G4endl;
...
void EventAction::SetReactionZPoint()
{
	// Make a reaction in the active volume [-110 mm, 110 mm]
	rZOfReactionInTarget = 110.0*(2*CLHEP::RandFlat::shoot()-1) * mm;
